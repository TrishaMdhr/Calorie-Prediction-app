...
def list_all_foods():
    with SessionLocal() as db:
        db_foods = crud.get_all_foods(db)
        return [_food_to_dict(f) for f in db_foods]


def update_food_item(food_id, **fields):
    with SessionLocal() as db:
        db_food = crud.update_food(db, food_id, **fields)
        return _food_to_dict(db_food)


def delete_food_item(food_id):
    with SessionLocal() as db:
        db_food = crud.delete_food(db, food_id)
        return _food_to_dict(db_food)
